...
def get_urls(url, download_number):
    try:
        headers = {
        ...
        img_list_wrap = main_wrap.find('div', {'class': 'list'})
        ...
        if img_list_wrap:
            # 提取 10 个包含商品信息的 div
            a_list = img_list_wrap.find_all('a')

            # 遍历列表，获取 div 里面实际包含 url 信息的 img 和跳转的 a 标签
            for a_ele in a_list:  # 取前十个商品
                image_url = 'http://www.netbian.com/' + a_ele['href']
